...
class DB {
  ...
  promptEmployees() {
    return this.connection.query(
      'SELECT CONCAT_WS (" ",first_name, last_name) as "name", id AS "value" FROM employee'
    );
  }
  ...
  managerPrompt() {
    return this.connection.query(
      'SELECT CONCAT_WS (" ",first_name, last_name) as "name", id AS "value" FROM employee WHERE role_id = 1 '
    );
  }
